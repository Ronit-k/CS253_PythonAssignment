...
def generate_invites(guest_list, event_details):
    ...
    Args:
        guest_list (list): A list of dictionaries, where each dictionary contains 'Name', 'Affiliation', and 'Email'.
        event_details (dict): A dictionary containing 'Date', 'Venue', and 'Schedule'.
    Returns:
    ...
    date = f"Date: {event_details['Date']}"
    venue = f"Venue: {event_details['Venue']}"
    schedule = f"Schedule: {event_details['Schedule']}"
    
    for guest in guest_list: # Iterates through the list of guests and creates an invitation for each guest
        name = guest['Name']
...
if __name__ == "__main__":
    ...
    results = generate_invites(guests, details)
